...
func TestLoadCell_LoadAugDict(t *testing.T) {
	...
	// The historical fixture predates strict C++ descriptor validation. Its root
	// has mask 1 although all references have mask 0; normalize only the test copy.
	if boc[12] != 0x23 {
		t.Fatalf("unexpected legacy root descriptor: %x", boc[12])
	}
	boc[12] = 0x03
	binary.LittleEndian.PutUint32(boc[len(boc)-4:], crc32.Checksum(boc[:len(boc)-4], castTable))

	c, err := FromBOCWithOptions(boc, BOCParseOptions{AllowNonZeroLevelRoot: true})
	if err != nil {
		t.Fatal(err)
		return
	}
