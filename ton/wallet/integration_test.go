...
func TestWallet_BroadcastTransactionsAndWait(t *testing.T) {
	...
	tx, block, err := w.BroadcastTransactionsAndWait(ctx, address.MustParseAddr("EQC9bWZd29foipyPOGWlVNVCQzpGAjvi1rGWF7EbNcSVClpA"), tlb.MustFromTON("0.005"), true, "Hello from tonutils-go!")
	if err != nil {
		t.Fatal("transfer err:", err)
	}
