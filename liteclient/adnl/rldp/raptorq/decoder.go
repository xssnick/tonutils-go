...
func (d *Decoder) AddSymbol(id uint32, data []byte) (bool, error) {
	...
	if id < d.params._K {
		// add fast symbol
		if d.symbols[id] != nil {
			return d.fastNum+d.slowNum >= d.params._K, nil
		}
		d.fastNum++
		d.symbols[id] = data
	}

	if d.fastNum+d.slowNum >= d.params._K+10 {
...
func (d *Decoder) Decode() (bool, []byte, error) {
	if d.fastNum+d.slowNum < d.params._K {
		return false, nil, fmt.Errorf("not enough symbols to decode")
	}

	if d.fastNum < d.params._K {
		...
		for k, v := range d.symbols {
			...
			if k >= d.params._K {
				...
				k = k + d.params._KPadded - d.params._K
			}
		...
		for i := uint32(len(toRelax)); i < d.params._KPadded; i++ {
		...
		relaxed, err := d.params.solve(toRelax)
		if err != nil {
		...
		for i := uint32(0); i < d.params._K; i++ {
			if d.symbols[i] != nil {
				continue
			}
			d.symbols[i] = d.params.genSymbol(relaxed, d.symbolSz, i)
		}
	}
